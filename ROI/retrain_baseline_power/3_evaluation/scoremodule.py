...
class ScoreModule:
    def __init__(self, inputs,workspace):
...
###---------------------------------模型預測前處理---------------------------------###        
        ...
        # best_model_list = data_type_checker(request['best_model_list']) 
        ...
        best_model_list = Dataset.get_by_name(workspace=self.ws, name=(f"{power_predict_name}-{data_result_plant}-best-perf").replace('_','-')).to_pandas_dataframe()
        ...
        if input_predict_type == 'baseline':
            ...
            # Process data
            data_result_sub, data_result_sub_temp = data_processor(data_result, 
                                                                   data_result_history,
                                                                   'baseline', 
                                                                   data_result_powertype, 
                                                                   best_model_list)
            ...
            predict_data_result_sub, predict_data_result_sub_temp = data_processor(predict_data_result,
                                                                                   data_result_history, 
                                                                                   'predict',
                                                                                   predict_data_result_powertype,
                                                                                   best_model_list)
        ...
        if power_predict_name=='predict_electricity':
            ...
            validate_performance = pd.merge(data_upload_final.drop(columns={'rec','year','month'}),
                                        data_result[['factory_electricity','ac_electricity','ap_electricity','production_electricity',
                                                     'base_electricity','plant','bo','datetime']]\
                                        .rename(columns={'factory_electricity':'predict_real',
                                                         'ac_electricity':'ac_real',
                                                         'ap_electricity':'ap_real',
                                                         'production_electricity':'production_real',
                                                         'base_electricity':'base_real'}),
                                        on = ['plant','bo','datetime'], how='inner')
        else:
            data_upload_final = data_upload[[power_predict_name,'plant','bo','year','month','datetime']]
            validate_performance = pd.merge(data_upload_final.drop(columns={'year','month'}),
                                        data_result[['ac_electricity','ap_electricity','production_electricity',
                                                     'base_electricity','plant','bo','datetime']]\
                                        .rename(columns={'ac_electricity':'ac_real',
                                                         'ap_electricity':'ap_real',
                                                         'production_electricity':'production_real',
                                                         'base_electricity':'base_real'}),
                                        on = ['plant','bo','datetime'], how='inner')
        ...
        power_predict_name_abbr = str(np.where(data_result_powertype=='空調用電（kwh）','ac',
                                          np.where(data_result_powertype=='空壓用電（kwh）','ap',
                                                  np.where(data_result_powertype=='生產用電（kwh）','production',
                                                           np.where(data_result_powertype=='基礎用電（kwh）','base','predict')))))
        validate_performance_final = pd.DataFrame({})
        validate_performance_sub = validate_performance[[power_predict_name_abbr+'_electricity',
                                                         power_predict_name_abbr+'_real','plant','bo']]\
                                                        .rename(columns={power_predict_name_abbr+'_real':'real_electricity',
                                                                         power_predict_name_abbr+'_electricity':'predict_electricity'})
        validate_performance_sub['real_electricity'] = validate_performance_sub.real_electricity.astype('float') 
        validate_performance_sub['mape'] = abs(validate_performance_sub.predict_electricity-validate_performance_sub.real_electricity)/validate_performance_sub.real_electricity
        validate_performance_sub = validate_performance_sub.groupby(['plant','bo'], group_keys=True).agg({'mape':'mean'}).reset_index()
        validate_performance_sub['power_type'] = power_predict_name_abbr+'_electricity'
        validate_performance_final = validate_performance_final.append(validate_performance_sub)
        validate_performance_final = validate_performance_final.astype('string')
        validate_performance_final_json =  json.loads(validate_performance_final.fillna('null').to_json(orient="records"))
        ...
        output_json = {
            ...
            'validate_performance':validate_performance_final_json
        }
...
def data_processor(data_result, data_result_history, predict_type, power_type, best_model_list):
    ...
    if predict_type=='predict':
        min2_date = min_date
    data_result_sub = data_result_sub.loc[(data_result_sub.datetime <= max_date) & \
                                          (data_result_sub.datetime >= min2_date),:].reset_index(drop=True)
    ...
    # best_model_path = os.path.join(os.path.dirname(os.path.realpath(__file__)),f"{plant}_{power_predict_name}_best_prediction_perf.csv") 
    # best_model_list = pd.read_csv(best_model_path) 
    ...
    data_result_sub_temp['temp_indicate'] = np.where(
        data_result_sub_temp['外氣平均溫度（℃）']<=data_result_sub_temp['Temp_cutoff'],1,0)
    # Lag 1 feature
    lag_1_time = np.unique(
        [(datetime.strptime(x,'%Y-%m-%d') + pd.DateOffset(months=-1)).strftime('%Y-%m-%d') for x in data_result_sub_temp['日期']]
    )
    data_result_sub_temp_lag = data_result_history.loc[data_result_history['日期'].astype(str).isin(lag_1_time),:]
    data_result_sub_temp_lag = data_result_sub_temp_lag[data_result_sub_temp_lag.columns[np.where(
        [x.find('kwh')!=-1 or x.find('plant')!=-1 or x.find('日期')!=-1 for x in data_result_sub_temp_lag.columns])]].reset_index(drop=True)
    data_result_sub_temp_lag.columns = ['日期','工廠用電（kwh）_lag1','空調用電（kwh）_lag1','空壓用電（kwh）_lag1',
                                        '生產用電（kwh）_lag1','基礎用電（kwh）_lag1','宿舍用電（kwh）_lag1','plant']
...
def model_predicting(best_model_name, data_colnames, data_result_sub, data_result_part, predict_model, predict_type):
    ...
    if best_model_name.find('lasso')!=-1 or best_model_name.find('rf')!=-1 or best_model_name.find('svm')!=-1:
        ...
        test_data = xgb.DMatrix(pd.DataFrame(data_result_part[predict_model.feature_names].astype(float).iloc[0,:])\
...
def model_predictor(data_result_sub, data_result_sub_temp,workspace_name,plant, predict_type, power_type): 
    ...
    model = azmodel(workspace_name, register_model_name).get_model_path(register_model_name, version=None, _workspace=workspace_name)
    print('we use model:\n',model)
    # print(data_result_part.columns)
